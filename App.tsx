...
import { AgentId, User, Agent, Notification, Doc } from './types';
...
const Sidebar: React.FC<{ user: User; activeAgentId?: string; onLogout: () => void }> = ({ user, activeAgentId, onLogout }) => {
  ...
  useEffect(() => {
    db.notifications.countUnread().then(setUnreadCount).catch(console.error);
    // Poll every minute?
    ...
    <div className="w-72 bg-slate-900 text-white flex flex-col h-full shrink-0 shadow-2xl relative z-20">
      ...
      <div className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        <div className="px-4 pb-4">
          ...
          <Link
            to="/app"
            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeAgentId === 'app'
          ...
          <Link
            to="/notifications"
            className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeAgentId === 'notifications'
          ...
          <Link
            to="/docs"
            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeAgentId === 'docs'
                ? 'bg-white/10 text-white ring-1 ring-white/20'
                : 'text-slate-400 hover:text-white hover:bg-white/5'
              }`}
          >
        ...
        {agents.map((agent) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all group relative overflow-hidden ${!hasAccess ? 'opacity-30 cursor-not-allowed' :
                  activeAgentId === agent.id
                    ? 'bg-white/10 text-white ring-1 ring-white/20'
                    : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}
            >
              <div className={`w-6 h-6 rounded-md flex items-center justify-center transition-all ${activeAgentId === agent.id ? 'bg-amber-500 text-slate-900' : 'bg-slate-800 text-slate-500'
                }`}>
        ...
        {user.role === 'ADMIN' && (
          <div className="pt-8">
            ...
            <Link
              to="/admin"
              className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeAgentId === 'admin'
...
const Dashboard: React.FC<{ user: User }> = ({ user }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [notifs, unread, docs, dashboardStats] = await Promise.all([
        db.notifications.findAll(),
        db.notifications.countUnread(),
        db.docs.findAll(),
    ...
    <div className="space-y-8 animate-fade-in pb-10">
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/10 p-6 rounded-2xl w-64">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-bold text-white">{stats?.gpa || 'N/A'}</span>
                </div>
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-bold text-white">{stats?.attendance || 'N/A'}</span>
                </div>
                <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <DashboardWidget
          ...
          icon="fa-bell"
          value={unreadCount > 0 ? t('dashboard.widget.newCount', { count: unreadCount }) : t('dashboard.widget.none')}
          subtext={latestNotification ? latestNotification.title : t('dashboard.widget.noNotifications')}
          color={unreadCount > 0 ? 'bg-rose-500' : 'bg-emerald-500'}
          to="/notifications"
        />
        <DashboardWidget
...
const Layout: React.FC<{ children: React.ReactNode; user: User; currentId?: string; onLogout: () => void; hideHeader?: boolean; contentClassName?: string }> = ({ children, user, currentId, onLogout, hideHeader, contentClassName }) => {
  ...
  useEffect(() => {
    ...
    const search = async () => {
      ...
      setGlobalResults(items.slice(0, 10));
    };

    const timeout = setTimeout(search, 300);
    return () => clearTimeout(timeout);
  }, [globalQuery, t, user.role, agents]);
...
const AgentView: React.FC<{ user: User; onLogout: () => void }> = ({ user, onLogout }) => {
  ...
  if (!agent) {
    // Fallback or loading? 
    // If agents are loaded, and not found -> 404
    ...
    <Layout user={user} currentId={agentId} onLogout={onLogout}>
      <div className="h-[calc(100vh-5rem-4rem)] min-h-0 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_420px] gap-6 max-w-7xl mx-auto w-full overflow-hidden">
        ...
        <div className="h-full min-h-0 flex flex-col gap-6 overflow-hidden">
          ...
          <div className="min-h-0 flex-1">
            {/* AgentToolsPanel likely needs update to handle async too, but for now passing userId */}
