...
function getGeminiApiKey(): string {
  ...
  const fromVite = (viteEnv?.VITE_GEMINI_API_KEY ||
...
export async function getAgentResponse(
  ...
  try {
    ...
    if (error.message?.includes("API_KEY")) {
      if (lang === "en")
        return "Security: API key is missing. Please contact an administrator.";
      if (lang === "kk")
        return "Қауіпсіздік: API кілті жоқ. Әкімшіге хабарласыңыз.";
