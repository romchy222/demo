...
function statusLabel(status: WorkflowCase['status']) {
  switch (status) {
    case 'OPEN':
      return 'Открыто';
    case 'IN_PROGRESS':
    ...
    <div className="h-full bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex flex-col">
      ...
      <div className="p-4 overflow-auto space-y-6">
        {agent.id === 'abitur' && (
          <>
            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Категория вопроса</div>
              <select
            ...
            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Быстрые запросы</div>
              <div className="flex flex-wrap gap-2">
                ...
                {quick.length === 0 && <div className="text-xs text-slate-400 font-bold">Нет быстрых кнопок</div>}
              </div>
            </div>

            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Справка</div>
              <div className="space-y-2">
                ...
                {reference.length === 0 && <div className="text-xs text-slate-400 font-bold">Справки нет</div>}
              </div>
            </div>
          </>
        )}
        ...
        {agent.id === 'kadr' && (
          <>
            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Кадровая тема</div>
              <select
                value={selectedTopic}
            ...
            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Разъяснения по процедурам</div>
              <div className="space-y-2">
                {procedures.map(p => (
                ...
                {procedures.length === 0 && <div className="text-xs text-slate-400 font-bold">Нет процедур</div>}
              </div>
            </div>
        ...
        {(agent.id === 'nav' || agent.id === 'room') && (
          <>
            <div>
              <div className="text-xs font-black text-slate-700 mb-2">{agent.id === 'nav' ? 'Учебный запрос' : 'Тип обращения'}</div>
              <select
              ...
              >
                ...
                {agent.id === 'nav' ? 'Запустить сценарий / подать заявление' : 'Создать обращение'}
              </button>
            </div>

            {agent.id === 'nav' && (
              <div>
                <div className="text-xs font-black text-slate-700 mb-2">Справочная информация</div>
                <div className="space-y-2">
                  {schedules.map(s => (
                  ...
                  {schedules.length === 0 && <div className="text-xs text-slate-400 font-bold">Нет данных</div>}
                </div>
              </div>
            )}

            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Статус обращений</div>
              <div className="space-y-2">
                {cases.map(c => (
                  <button
                    key={c.id}
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{statusLabel(c.status)}</div>
                    </div>
                    <div className="text-[11px] text-slate-500 font-semibold mt-1">{new Date(c.createdAt).toLocaleString()}</div>
                  </button>
                ))}
                {cases.length === 0 && <div className="text-xs text-slate-400 font-bold">Обращений пока нет</div>}
              </div>
            </div>
            ...
            {activeCase && (
              <div className="border border-slate-200 rounded-2xl overflow-hidden">
                <div className="p-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-black text-slate-800">Обращение</div>
                ...
                <div className="p-3 space-y-2 max-h-56 overflow-auto">
                  {caseMessages.map(m => (
                    <div key={m.id} className={`text-xs rounded-xl px-3 py-2 ${m.authorRole === 'ADMIN' ? 'bg-emerald-50 text-emerald-900' : 'bg-slate-100 text-slate-800'}`}>
                      <div className="font-black text-[10px] uppercase tracking-widest opacity-70 mb-1">{m.authorRole}</div>
                      <div className="whitespace-pre-wrap">{m.message}</div>
                    </div>
                  ))}
                  {caseMessages.length === 0 && <div className="text-xs text-slate-400 font-bold">Сообщений нет</div>}
                </div>
                <div className="p-3 border-t border-slate-200 bg-white">
                  <textarea
                    ...
                    placeholder="Уточнение / сообщение..."
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <button
                      ...
                      title="Попросить ИИ помочь сформулировать"
                    >
                      ИИ
                    </button>
                  </div>
        ...
        {agent.id === 'career' && (
          <>
            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Карьерное направление</div>
              <select
            ...
            <div>
              <div className="text-xs font-black text-slate-700 mb-2">Подсказки по резюме</div>
              <div className="space-y-2">
                {resumeTips.map(tip => (
                ...
                {resumeTips.length === 0 && <div className="text-xs text-slate-400 font-bold">Нет подсказок</div>}
              </div>
            </div>
          </>
        )}
